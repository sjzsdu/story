...
- 分发：Vite 构建到 `web/dist/`，由 `web/embed.go` 的 `go:embed` 打进单二进制；`story serve`（默认 127.0.0.1:7878）一条命令启动。`web/dist/` 须入库（至少保留占位 index.html 让无 Node 环境也能 go build）；`web/node_modules/` 不入库。
