...
// Provider 小红书发布 provider（sau CLI 包装）。
// sau 支持小红书的视频上传和图文笔记上传。
...
func (p *Provider) Platform() domain.Platform { return domain.PlatformXiaohongshu }

func (p *Provider) Upload(ctx context.Context, req port.PublishRequest) (*port.PublishResult, error) {
	if req.Account == nil {
		return nil, fmt.Errorf("小红书发布需要账号凭证")
	}
